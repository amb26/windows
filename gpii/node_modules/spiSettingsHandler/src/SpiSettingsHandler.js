...
windows.user32 = new ffi.Library('user32', {
...
windows.spi.structures = {
    HighContrast: {
        ...
        dwFlags: {
            ...
            conversion: {
                type: "flags",
                flags: ["HCF_AVAILABLE", "HCF_CONFIRMHOTKEY", "HCF_HIGHCONTRASTON", 
...
// A cache of computed data based on "structures" above
windows.spi.cached = {
...
fluid.merge(null, windows.API_constants, windows.spi.constants);
...
windows.spi.schemaToStruct = function (structName) {
    var path = fluid.path(structName, "ffi-struct");
    var togo = fluid.get(windows.spi.cached, path);
    if (!togo) {
        togo = [];
        var struct = windows.spi.structures[structName];
...
windows.spi.getValue = function (action, uiParam, structName) {
    ...
    var callSuccessful = windows.user32.SystemParametersInfoW(action, uiParam, pvParam.ref(), 0);
    console.log("Returned ", pvParam, ": ", pvParam.lpszDefaultScheme);
    console.log("Default scheme: ", windows.fromWideChar(pvParam.lpszDefaultScheme));
    if (!callSuccessful) {
...
windows.spi.setValue = function (action, uiParam, structName, value) {
    ...
    //pvParam.lpszDefaultScheme = windows.toWideChar("High Contrast Black (large)");
    var struct = windows.spi.structures.HighContrast;
    for (var key in struct) {
    ...
    var callSuccessful = windows.user32.SystemParametersInfoW(action, uiParam, pvParam.ref(), c.SPIF_SENDCHANGE);
    if (!callSuccessful) {
